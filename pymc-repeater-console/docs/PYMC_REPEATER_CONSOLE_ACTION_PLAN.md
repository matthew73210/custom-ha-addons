...
| Upstream release refs | `Dockerfile`, `.github/workflows/build-pymc-repeater-console.yml`, `compatibility/upstream-watch.json` | Dockerfile and publishing workflow defaults use reviewed commit SHAs for pyMC Repeater, pyMC Core, and pyMC Console dist. The watch manifest compares those pins with upstream `main`, while build args can override refs temporarily for review or recovery. | OK | Release builds remain reproducible while scheduled/manual CI reports whether watched upstream SHAs look safe to pin manually. | Keep release pins aligned across the Dockerfile, publishing workflow, and watch manifest. Update them only through maintainer-reviewed commits. | Medium | Build/CI behavior only |
...
- Scheduled/manual **Check pyMC Upstream Mergeability** workflow compares reviewed pins with manifest watched refs and tests exact watched SHAs without publishing release images.
- The watch manifest tracks pyMC Repeater, pyMC Core, and pyMC Console dist; watched refs may be `main`, `dev`, or another reviewed branch.
- CI failure text distinguishes wrapper validation failures, default upstream build/test failures, upstream drift, inconclusive infrastructure checks, and merge-readiness contract changes.
...
- Should releases pin upstream commits while CI tests watched refs? Current answer: **yes; Dockerfile and publishing workflow defaults use reviewed SHAs, and scheduled/manual upstream-watch CI tests exact watched SHAs without publishing**.
...
- Upstream refs default to reviewed commit SHAs for release/default builds.
- Scheduled/manual CI compares those pins with watched refs and tests drifted SHAs without publishing release images.
- Contract tests pass.
